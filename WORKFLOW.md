...
글을 미리 써두고 **미래 날짜**로 공개할 수 있습니다.

```markdown
---
pubDate: 2026-09-20     ← 이 날짜가 되면 자동 공개
---
```

- 배포된 사이트에는 **오늘 이전 날짜의 글만** 나옵니다
- 로컬 `npm run dev` 에서는 예약 글도 보입니다 (미리보기용)
- **매일 한국시간 오전 9시 10분**, GitHub Actions 가 그날 공개할 글이 있는지 확인하고
  있으면 자동으로 재배포합니다 (`.github/workflows/daily-publish.yml`)
- 설정할 것은 없습니다. 별도 토큰이나 비밀값이 필요하지 않습니다

**즉시 공개하고 싶으면** 두 가지 방법이 있습니다.

1. `pubDate` 를 오늘 날짜로 바꾸고 `npm run deploy`
2. GitHub 저장소 → **Actions** 탭 → `daily-publish` → **Run workflow**

> GitHub 무료 플랜의 예정 작업은 정시보다 **수십 분 늦게** 실행될 수 있습니다.
> 그리고 저장소에 60일간 아무 활동이 없으면 예정 작업이 자동 중지되니,
> 글을 꾸준히 올리는 동안은 문제가 없습니다.
