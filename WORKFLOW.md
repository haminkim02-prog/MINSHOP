...
| **물결표(`~`)가 사라지고 글자에 취소선이 생김** | 아래 참고 |

### ⚠️ 물결표(`~`) 주의

마크다운에서 `~`는 **취소선 문법**입니다. 한 문단에 물결표가 짝으로 들어가면
그 사이가 취소선이 되고 물결표가 사라집니다.

```
잘못:  EDT(5~15%)와 EDP(15~20%)
결과:  EDT(5̶1̶5̶%̶)̶와̶ ̶E̶D̶P̶(̶1̶5̶20%)     ← 물결표 소실 + 취소선

올바름: EDT(5\~15%)와 EDP(15\~20%)
결과:  EDT(5~15%)와 EDP(15~20%)      ← 정상
```

**범위를 나타내는 물결표는 항상 `\~` 로 쓰세요.** (백슬래시 + 물결표)

- `3\~5시간`, `1\~2회`, `5\~15%`
- 예외: ` ``` ` 코드블록 안에서는 그냥 `~`를 써도 됩니다
- 헷갈리면 그냥 `\~`로 통일하세요. 화면 출력은 똑같습니다
